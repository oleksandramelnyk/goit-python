...
представляет собой словарь со следующими ключами: фамилии авторов – ключ "author", название статьи – ключ "title",
...
def find_articles(key, letter_case=False):
    ...
    for blog in articles_dict:
        if letter_case is False:
            if key.lower() in blog["title"].lower() or key.lower() in blog["author"].lower():
                articles.append(blog)
        else:
            if key in blog["title"] or key in blog["author"]:
                articles.append(blog)
    return articles
